...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://yapcasia.org/mt-comments.cgi?__mode=session_js&blog_id=3&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = 'e81c3a5ee3321c7907c6a8b8ee2c337dbc7e3e47';
...
function mtSignIn() {
    ...
    var url = 'http://yapcasia.org/mt-cp.cgi?__mode=login&blog_id=3';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fyapcasia.org%2F2011%2F';
        }
    } else {
